...
class PohonKeluargaApp:
    ...
    def hapus_node(self):
            nama_input = simpledialog.askstring("Hapus Anggota", "Masukkan nama yang ingin dihapus:")
            if not nama_input:
                return

            nama_input = nama_input.strip().lower()

            for node in self.pohon.nodes:
                nama1, nama2 = node.pasangan

                nama1_clean = nama1.split(' (')[0].strip().lower() if nama1 else ''
                nama2_clean = nama2.split(' (')[0].strip().lower() if nama2 else ''

                if nama1_clean == nama_input:
                    node.pasangan = ('', nama2)
                elif nama2_clean == nama_input:
                    node.pasangan = (nama1, '')
                else:
                    continue  # lanjutkan ke node berikutnya

                # Jika pasangan kosong, hapus node dan relasi dari parent
                if node.pasangan == ('', ''):
                    # Hapus dari daftar anak orang tua mana pun
                    for parent in self.pohon.nodes:
                        if node in parent.anak:
                           parent.anak.remove(node)
                ...
                self.update_canvas()
                messagebox.showinfo("Berhasil", f"Nama '{nama_input}' dihapus.")
                return  # Hentikan setelah satu nama dihapus

            messagebox.showwarning("Tidak Ditemukan", f"Nama '{nama_input}' tidak ditemukan.")
    ...
    def pilih_node(self, event):
        x, y = event.x / self.scale, event.y / self.scale
        for node in self.pohon.nodes:
            node_x, node_y = node.canvas_x, node.canvas_y
            if abs(x - node_x) <= 60 and abs(y - node_y) <= 20:
                self.node_terpilih = node
                self.update_canvas()
                return
        self.node_terpilih = None
        self.update_canvas()
    ...
    def drag_node(self, event):
        if not self.fixed_layout:
            return
        if self.node_terpilih:
            dx = (event.x - self.drag_data["x"]) / self.scale
            dy = (event.y - self.drag_data["y"]) / self.scale
            self.node_terpilih.x += dx
            self.node_terpilih.y += dy
            self.drag_data["x"] = event.x
            self.drag_data["y"] = event.y
            self.update_canvas()
